...
def train_model( x, x_val, class_weights = None, train_path = None, train_index = None, imagenet = True):
    # checkpoint

    if train_index == None:
        if os.path.isfile('pests.h5'):
            model = load_model_first()
            model.load_weights('pests.h5')
        else:
    ...
    es = EarlyStopping(monitor='val_f1', patience=20, mode='auto', min_delta=0.001)
...
def dfprocessing_func(data):
    ...
    if expoent_index != None:
        ...
        train_path = train_path + "_expoent_index_whole"
